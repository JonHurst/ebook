...
def process_doubles(p):
    ...
    #double preceded by space or ' and followed by word character or ' is an opening double
    p = re.sub('(\\s+|\')\"([\\w\'])', r'\1“\2', p)
    #double preceded by a character that is not a space
    #and followed by space|'|[—,:;.] is a closing double
    p = re.sub('([^\\s])\"(\\s+|[\'—\.,:;])', r'\1”\2', p)
...
def process_para(p):
    ...
    if False: #do when flag set (TODO)
        quote_counter = 0
        for c in p:
